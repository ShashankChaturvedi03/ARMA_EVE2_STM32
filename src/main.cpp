...
void button1(){
  ...
  Send_CMD(CLEAR_COLOR_RGB(0, 0, 0));
  Send_CMD(CLEAR(1, 1, 1));
  Cmd_Button(380,200,100,100,30,0,"2");
  Send_CMD(DISPLAY());
  Send_CMD(CMD_SWAP);
  UpdateFIFO(); // Trigger the CoProcessor to start processing commands out of the FIFO

  // volatile int Tag1 = rd8(REG_TOUCH_TAG + RAM_REG);
  wait_us(50000);
  // write_ARMA();
...
void blink(){
  ...
  Send_CMD(CLEAR_COLOR_RGB(0, 0, 0));
  Send_CMD(CLEAR(1, 1, 1));
  Cmd_Text(380,200,31,0,"LED Toggled");
   Send_CMD(DISPLAY());
  Send_CMD(CMD_SWAP);
  UpdateFIFO(); 
...
void button(){
  ...
  Send_CMD(CLEAR(1, 1, 1));
  Send_CMD(TAG(1));
  Cmd_Button(380,200,200,100,30,0,"Toggle LED");
  // Cmd_Text(380,200,31,0,"Shashank");
  Send_CMD(TAG(0x300));
  Send_CMD(DISPLAY());
  Send_CMD(CMD_SWAP);
  UpdateFIFO(); // Trigger the CoProcessor to start processing commands out of the FIFO

  volatile int TOUCH_RAW_XY  = 0;
  volatile int TOUCH_SCREEN_XY = 0;
  volatile int TOUCH_TAG = 0;
  volatile int TOUCH_TAG_XY = 0;

  printf("TOUCH_RAW_XY: %d",TOUCH_RAW_XY);
  printf("TOUCH_SCREEN_XY: %d",TOUCH_SCREEN_XY);
  printf("TOUCH_TAG: %d",TOUCH_TAG);
  printf("TOUCH_TAG_XY: %d",TOUCH_TAG_XY);

  wait_us(50000);
  // write_ARMA();
  while(TOUCH_SCREEN_XY < 1){
   TOUCH_RAW_XY  = rd8(REG_TOUCH_RAW_XY + RAM_REG);
  TOUCH_SCREEN_XY = rd8(REG_TOUCH_SCREEN_XY + RAM_REG);
   TOUCH_TAG = rd8(REG_TOUCH_TAG + RAM_REG);
   TOUCH_TAG_XY = rd8(REG_TOUCH_TAG_XY + RAM_REG);
  }
